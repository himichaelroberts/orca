...
describe('terminal.recoverPane liveness gate', () => {
  it('recreates a shell for an SSH pane the relay disowned, the only evidence this gate ever gets', async () => {
    ...
    // answer in the inventory listing below — and that answer is a union: pty-handler.ts:1936
    // throws not-found for an unknown id with no liveness check, and pty.listProcesses returns only
